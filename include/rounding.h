...
template <typename NT, class MT, class VT, class Polytope, class Parameters>
std::pair< std::pair<MT, VT>, NT> comp_lintrans(Polytope &P, Parameters &var) {
    ...
    if (!P.get_points_for_rounding(randPoints)) {  // If P is a V-polytope then it will store its vertices in randPoints
        ...
        // 3. Sample points from P
        randPoints.clear();
        rand_point_generator(P, p, 10*n, walk_len, randPoints, var); // sample >10*n in order to estimate ratio between radius of largest inscribed ball and smallest enclosing
...
template <class Polytope, class MT, class VT, typename NT, class Parameters>
std::pair< std::pair<MT, VT>, NT> is_last_rounding(Polytope &P2,
                                                   std::pair<MT, VT> Ell,
                                                   NT ratio,
                                                   NT &round_value,
                                                   bool &comp_next,
    ...
    std::pair< std::pair<MT, VT>, NT> res = comp_lintrans<NT, MT, VT>(P3, var);
...
template <typename NT, class Polytope, class Parameters>
std::pair<NT,NT> round_polytope(Polytope &P, Parameters &var) {

    typedef typename Polytope::MT 	MT;
    typedef typename Polytope::VT 	VT;
    //typedef typename Polytope::PolytopePoint Point;
    std::pair< std::pair<MT, VT>, NT> res;
    res = comp_lintrans<NT, MT, VT>(P, var);
    bool comp_next = true;
