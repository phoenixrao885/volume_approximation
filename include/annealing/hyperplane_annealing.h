...
void check_converg(HPolyBall &Si, PointList &randPoints, NT p_test, bool &done, bool &too_few, Parameters &var, bool print) {
...
void reconstruct_hyp(HPolyBall &Si, VPolytope &VP, std::vector<HPolyBall> &PolyBallSet,
    ...
    while (true) {
        ...
        while (true) {
            ...
            check_converg<Point>(SiIter, randPoints, p_value, doneIter, too_few, var, false);
...
void construct_new_hyp(HPolyBall &Si, VPolytope &VP, std::vector<HPolyBall> &PolyBallSet,
    ...
    check_converg<Point>(SiTemp, randPoints, p_value, done, too_few, var, false);
    ...
    if (too_few) {
        ...
        reconstruct_hyp(Si, VP, PolyBallSet, randPoints, a, z0, v, p_value, var);
        //added_in_set = true;
    }
...
void get_seq_convex(Vpolytope &VP, std::vector<HPolyBall> &PolyBallSet, NT p_value, NT epsilon,
   ...
   // Point direction(n), cent(n), qbound(n);
    ...
    while (true) {
        ...
        do {
            ...
            if (count>=thre) {
                if (Si.num_of_hyperplanes() > PolyBallSet[PolyBallSet.size()-1].num_of_hyperplanes()) {
                    ...
                    check_converg<Point>(VP, randPoints, epsilon, done, too_few, var, false);
                }
        ...
        construct_new_hyp(Si, VP, PolyBallSet, randPoints, q, p_value, var);
...
void get_hyperplane_annealing(Vpolytope &VP, std::vector<HPolyBall> &PolyBallSet,
                              NT &p_value, NT &a, NT epsilon, Parameters &var) {
    ...
    bool done = false;
    get_seq_convex(VP, PolyBallSet, p_value, epsilon, var, done);
