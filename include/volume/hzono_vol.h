...
NT vol_hzono (Zonotope &ZP, UParameters &var, AParameters &var_ban, GParameters &var_g,
              std::pair<Point,NT> &InnerB, NT &nballs, Rcpp::Function diam_zono, bool only_phases = false) {
    ...
    NT lb = var_ban.lb, ub = var_ban.ub, prob = var_ban.p, rmax = var_ban.rmax,
            radius = InnerB.second, round_value = 1.0, e = var.error, ratio, vol, alpha = var_ban.alpha, diam0 = var.diameter;
    ...
    get_sequence_of_zonopolys<ZonoHP>(ZP, HP2, HPolySet, Zs_max, ratios, N*nu, nu, lb, ub, alpha, var, diam_zono, diams_inter);
    var.diameter = diam0;
    ...
    if (!window2) {
        ...
        vol *= esti_ratio_interval<RNGType>(HP2, ZP, ratio, er0, win_len, N*nu, prob, Point(n), var2);
    } else {
    ...
    if (HPolySet.size()==0) {
        ...
        if (ratios[0]!=1) {
            if(!window2) {
                vol = vol / esti_ratio_interval<RNGType>(ZP, HP2, ratios[0], er1, win_len, N*nu, prob, Point(n), var);
            } else {
        ...
        b1 = HPolySet[0];
        if(!window2) {
            vol = vol / esti_ratio_interval<RNGType>(ZP, b1, ratios[0], er1, win_len, N*nu, prob, Point(n), var);
        } else {
        ...
        for (int i = 0; i < HPolySet.size()-1; ++i) {
            ...
            if(!window2) {
                ...
                std::cout<<"stored diameter = "<<var.diameter<<std::endl;
                vol = vol / esti_ratio_interval<RNGType>(zb1, b2, ratios[i], er1, win_len, N*nu, prob, Point(n), var);
            } else {
        ...
        if (!window2) {
            ...
            std::cout<<"stored diameter = "<<var.diameter<<std::endl;
            vol = vol / esti_ratio_interval<RNGType>(zb1, HP2, ratios[ratios.size() - 1], er1, win_len, N*nu, prob, Point(n), var);
        } else {
